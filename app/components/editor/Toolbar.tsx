...
export default function Toolbar() {
  ...
  return (
    <div className="flex items-center gap-2 px-4 h-11 border-b bg-(--editor-bg-toolbar) border-(--editor-border-color)">
      {/* Style button (visual) */}
      <button
        className={`${btnBase} flex items-center gap-1.5 bg-(--editor-brand-primary) text-(--editor-text-on-brand)`}
      >
      ...
      <div className="w-px h-6 mx-1 bg-(--editor-border-color)" />

      {/* Undo / Redo (functional) */}
      <button onClick={handleUndo} className={`${iconBtn} text-(--editor-text-secondary)`} title="Undo">
        <svg className="w-4 h-4" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
          <polyline points="1 4 1 10 7 10"/>
          <path d="M3.51 15a9 9 0 1 0 2.13-9.36L1 10"/>
        </svg>
      </button>
      <button onClick={handleRedo} className={`${iconBtn} text-(--editor-text-secondary)`} title="Redo">
        <svg className="w-4 h-4" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
      ...
      <button
        ...
        className={`${iconBtn} ${activeDevice === 'mobile' ? 'bg-(--editor-brand-primary-light) text-(--editor-brand-primary)' : 'bg-transparent text-(--editor-text-secondary)'}`}
        title="Mobile"
      >
        <svg className="w-4 h-4" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
      ...
      <button
        ...
        className={`${iconBtn} ${activeDevice === 'desktop' ? 'bg-(--editor-brand-primary-light) text-(--editor-brand-primary)' : 'bg-transparent text-(--editor-text-secondary)'}`}
        title="Desktop"
      >
        <svg className="w-4 h-4" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
      ...
      <button className={`${btnBase} flex items-center gap-1.5 border border-(--editor-border-color) text-(--editor-text-primary)`}>
        <svg className="w-4 h-4" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
      ...
      <button className="text-sm flex items-center gap-1 text-(--editor-brand-primary-text)">
      ...
      <button className="text-sm flex items-center gap-1 ml-4 text-(--editor-brand-primary-text)">
